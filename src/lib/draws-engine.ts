...
const MONTHLY_CONTRIBUTION_CENTS = 999;
const POOL_SHARE_OF_REVENUE = 0.5;
...
export async function simulateDrawClient(drawId: string): Promise<SimResult> {
  return await buildSimulation(drawId);
}

export async function publishDrawClient(
  drawId: string,
): Promise<SimResult & { rolloverToNext: number; winnersCreated: number }> {
  const sim = await buildSimulation(drawId);
  ...
  // If rollover, push it to the next draft/open draw
  if (rollover > 0) {
    const { data: thisDraw } = await supabase
      .from("draws")
      .select("draw_month")
      .eq("id", sim.drawId)
      .single();
